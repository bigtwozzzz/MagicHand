...
class Trainer:
    ...
    def _save_snapshot(self):
        ...
        save_name = f"{self.config.model.name}_epoch_{self.best_state['epoch']}_{self.metric_name}_{metric_score:.2}_loss_{self.best_state['loss']:.2}.pth"
    ...
    def test(self):
        ...
        with Logger("Test", self.max_epoch, len(self.test_data), self.config.log_every, self.gpu_id) as logger:
            for iteration, (images, targets) in enumerate(self.test_data):
                ...
                # Fix CUDA OOM by moving data to CPU for metric calculation
